...
// VARIABEL POSISI AWAL KAMERA
float kameraX = 100.0f; //posisi kamera x
float kameraY = 200.0f; //posisi kamera y
float kameraZ = 100.0f; //posisi kamera z

// VARIABEL KONTROL MOBIL
float mobilX = 80.0f; //titik koordinat x
float mobilY = 5.0f;  //titik koordinat y
float mobilZ = 75.0f; //titik koordinat z
float mobilRotasi = 90.0f;  //sudut rotasi mobil
float kecepatan_mobil = 2.0f; //kecepatan maju mundur
float kecepatan_belok = 5.0f; //kecepatan belok

// VARIABEL STATUS LAMPU
bool lampOn = false; //lampu mati

// Skala global untuk pohon (dapat dimanipulasi via + / -)
float ukuranpohon = 8.0f;
...
void bangunan(){
  ...
  Kubus(60,43,21,0,0,0,140,2,4); //palang lantai 2
  glPopMatrix();
...
void jalan () {
  ...
  Kubus(0,-2,150,0,0,0,300,0.5,140);
  glPopMatrix();
}
...
void display() {
  ...
  //===============Pohon==================
  glPushMatrix();
  glTranslatef(-135,-3,125);
  glScalef(ukuranpohon, ukuranpohon, ukuranpohon);
  pohon();
  glPopMatrix();

  glPushMatrix();
  glTranslatef(-105,-3,125);
  glScalef(ukuranpohon, ukuranpohon, ukuranpohon);
  pohon();
  glPopMatrix();

  glPushMatrix();
  glTranslatef(-75,-3,125);
  glScalef(ukuranpohon, ukuranpohon, ukuranpohon);
  pohon();
  glPopMatrix();

  glPushMatrix();
  glTranslatef(-45,-3,125);
  glScalef(ukuranpohon, ukuranpohon, ukuranpohon);
  pohon();
  glPopMatrix();

  glPushMatrix();
  glTranslatef(-15,-3,125);
  glScalef(ukuranpohon, ukuranpohon, ukuranpohon);
  pohon();
  glPopMatrix();
...
void keyboard(unsigned char key, int x, int y) {
    ...
    if (key == '1') { // depan
        kameraX = 0.5f; kameraY = 200.0f; kameraZ = 350.0f;
    }
    if (key == '2') { // kanan
        kameraX = 250.0f; kameraY = 200.0f; kameraZ = 250.5f;
    }
    if (key == '3') { // kiri
        kameraX = -250.0f; kameraY = 200.0f; kameraZ = 250.0f;
    }
    if (key == '4') { // atas
        kameraX = 0.0f; kameraY = 400.0f; kameraZ = 100.0f;
    }

    // --- SKALA POHON (+ / -)
    if (key == '+' | key == '=') { //menambah ukuran pohon
        ukuranpohon += 0.1f;
    }
    if (key == '-') { // mengurangi ukuran pohon
        ukuranpohon -= 0.1f;
        if (ukuranpohon < 0.1f) ukuranpohon = 0.1f; //batas minimum ukuran pohon
    }

    // kontrol lampu (L ON, O OFF)
...
void init() {
  ...
  glClearColor(0.0f, 0.0f, 0.2f, 0.5f); //Latar Belakang
