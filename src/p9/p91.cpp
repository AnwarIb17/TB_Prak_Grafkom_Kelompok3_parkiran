...
// Variabel Kontrol Vertical Key (PERBAIKAN KONFLIK)
bool isCtrlLDown = false;
bool isCtrlRDown = false;
...
void drawTeapot() {
    ...
    glPushMatrix();
        // Ganti objek teko menjadi kubus.
        // Kubus dengan sisi 1.0 akan ditranslasikan setengah tinggi (0.5)
        // agar tampak berada di atas lantai (y = 0).
...
void updateCamera() {
    ...
    // Vektor pergerakan horizontal (hanya menggunakan Yaw)
    float moveForwardX = sin(yawRad);
    float moveForwardZ = -cos(yawRad);
    
    // Vektor samping (right)
    float rightX = sin(yawRad + PI/2);
    float rightZ = -cos(yawRad + PI/2);

    // Pergerakan Maju (W) - Hanya di bidang XZ
    if (keys['w'] || keys['W']) {
        cameraPosX += moveForwardX * MOVE_SPEED;
        cameraPosZ += moveForwardZ * MOVE_SPEED;
    }
    // Pergerakan Mundur (S) - Hanya di bidang XZ
    if (keys['s'] || keys['S']) {
        cameraPosX -= moveForwardX * MOVE_SPEED;
        cameraPosZ -= moveForwardZ * MOVE_SPEED;
    }
    // Pergerakan Samping (A dan D)
    if (keys['a'] || keys['A']) {
    ...
    // Pergerakan Vertikal (Naik/Turun)
    if (keys[' ']) cameraPosY += MOVE_SPEED;
    // MENGGUNAKAN FLAG BOOL TERPISAH (PERBAIKAN KONFLIK)
    if (isCtrlLDown || isCtrlRDown) cameraPosY -= MOVE_SPEED; 
}
...
void display() {
    ...
    // Vektor pandangan penuh (untuk melihat)
...
void specialKey(int key, int x, int y) {
    // Digunakan untuk tombol non-ASCII seperti CTRL
