...
// Array keys untuk menampung status tombol ASCII
bool keys[256]    = {false}; 
...
void updateLightingLogic() { 
    ...
    GLfloat w_component = isDirectional ? 0.0f : 1.0f; // 0.0f directional, 1.0f positional
...
void updateCamera() { 
    ...
    // Kontrol Vertikal (Space untuk naik, 'c' untuk turun)
    if (keys[' ']) {
...
void keyboard(unsigned char key, int x, int y) { 
    ...
    // Mengubah Linear Attenuation (N/M)
    if (key == 'n' || key == 'N') { 
...
// specialKey dan specialKeyUp sekarang kosong karena kontrol Ctrl sudah diubah
...
int main(int argc, char** argv) { 
    ...
    // BARIS GL CLEAR COLOR SUDAH DITUTUP DENGAN BENAR
